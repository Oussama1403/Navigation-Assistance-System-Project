...
def process_frames(frame_queue, result_queue,classes,classes_openimages,yolo_weights_file,openimages_weights_file,yolo_config_file,openimages_config_file):
    ...
    while True:
        ...
        openimages_class_ids, openimages_confidences, openimages_boxes = detect_objects(openimages_model, frame, classes_openimages)
        ...
        annotated_frame = draw_boxes(annotated_frame, openimages_class_ids, openimages_confidences, openimages_boxes, classes_openimages)
...
def main():
    ...
    openimages_weights_file = os.path.join(current_directory,"Camera Video Analysis Module","yolo","yolov3-openimages.weights")
    openimages_config_file = os.path.join(current_directory, "Camera Video Analysis Module", "yolo", "cfg", "yolov3-openimages.cfg")
    ...
    # Load class names 
    classes_openimages = []
    with open(os.path.join(current_directory, "Camera Video Analysis Module", "yolo", "data", "openimages.names"), "r") as f:
        classes_openimages = [line.strip() for line in f.readlines()]

    frame_queue = multiprocessing.Queue(maxsize=2)  # Adjust maxsize for buffering
    result_queue = multiprocessing.Queue(maxsize=1)

    process = multiprocessing.Process(target=process_frames, args=(frame_queue, result_queue, classes,classes_openimages,yolo_weights_file,openimages_weights_file,yolo_config_file,openimages_config_file))
    process.start()
